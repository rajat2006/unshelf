...
import type { Item, ItemDetail, LearningPlanView } from "@unshelf/shared";
import {
  startTestApp,
  TEST_USER_HEADER,
  type TestApp,
...
describe("DELETE /api/items/:itemId", () => {
  it("atomically removes active organisation while preserving frozen history and scalar facts", async () => {
    ...
    await request(app)
      .patch(
        `/api/items/${item.id}/parts/${(parts.body as ItemDetail).parts[0].id}/completion`,
      )
      .set(TEST_USER_HEADER, user)
      .send({ completed: true });
    await request(app)
    ...
    const after = await harness.pool.query<{ deleted_at: Date }>(
      `select id, user_id, title, source, created_at, type, status,
  ...
  it("removes direct and staged placements while preserving surrounding topology", async () => {
    ...
    const remainingStaged = await request(app)
      ...
      .set(TEST_USER_HEADER, user)
      .expect(200);
    expect((remainingDirect.body as LearningPlanView).nodes).toEqual([
      expect.objectContaining({ id: beforeNode }),
      expect.objectContaining({ id: afterNode }),
    ]);
    expect((remainingDirect.body as LearningPlanView).edges).toEqual([]);
    ...
    expect(retained.rows).toEqual([{ placements: 0, origins: 0 }]);
  });

  it("releases exact Provider identities and Keeps while preserving other Discover data", async () => {
    const user = "delete-item-discover";
    const item = (await capture(user, { title: "Kept Item", type: "video" }))
      .body as Item;
  ...
  it("replays deletion without repeating cleanup or changing its timestamp", async () => {
