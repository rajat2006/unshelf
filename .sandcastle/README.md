...
- **`resolve-agent.ts`** — `resolveAgent(labels, capability)` (Unshelf-specific):
  ...
  while Codex uses `gpt-6-sol` for Build and `gpt-6-astra` for Think, both at
  `medium`. Effort types are derived from Sandcastle's own factory
  ...
  |---|---|---|---|---|
  | `implement` | `claude-opus-4-8` | `medium` | `gpt-6-sol` | `medium` |
  | `implement-prd` | `claude-opus-4-8` | `medium` | `gpt-6-sol` | `medium` |
  | `implement-pr` | `claude-opus-4-8` | `medium` | `gpt-6-sol` | `medium` |
  | `update-branch` | `claude-opus-4-8` | `medium` | `gpt-6-sol` | `medium` |
...
- **`prepare-codex-auth.ts`** — `prepareCodexAuth(providerName)` (Unshelf-specific):
  ...
  uses the ChatGPT subscription seat, not the metered API. A no-op when the
