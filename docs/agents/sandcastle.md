...
| `agent-implement-pr.yml` | `agent:implement-pr` on a PR (`pull_request_target`) | Checks out the PR branch and runs the `implement-pr` capability: it reads the PR's review threads, **changes what it safely can and commits**, running the repo's typecheck/test on what it touches, and marks each comment `addressed` or `deferred`. The workflow pushes the fix commits and posts one summary comment. It does **not** flip the PR's draft/ready state — that belongs to `agent-review`. Swaps `agent:implement-pr` → `agent:in-progress` on start; a failure lands the PR in `agent:blocked` with a run-URL comment and always removes `agent:in-progress`. Shares the same per-PR concurrency group as `agent-review` (`queue: max`, `cancel-in-progress: false`), so the two never mutate a PR concurrently. Fork PRs are refused. |
