...
| `agent-promote-queued.yml` | An issue closing (`issues: closed`) | **Pure label automation — no runner, no checkout.** Finds every dependent the closed issue was blocking (GitHub native issue dependencies) and flips each waiting `agent:queued` issue to `agent:implement` via `AGENT_PAT`, so the promotion chains straight into `agent-implement`. Promotes only when the dependent is still `agent:queued`, is not `ready-for-human`, and has **no other open blocker** — a dependent with a second unmet blocker stays `agent:queued`. The decision is the pure `decidePromotion` in `.sandcastle/promote-queued.ts`, mirrored in the workflow's `gh`/`jq` bash. |
