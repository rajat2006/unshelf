...
| `agent-architecture-review.yml` | weekly cron + `workflow_dispatch` (**no label**) | Checks out `main` and runs the `architecture-review` capability (the repo's local `/improve-codebase-architecture`, no external skills registry) to sweep the whole tree for codebase-level **drift** (vs `CONTEXT.md`/ADRs) and **deepening opportunities**. Read-only — the agent commits nothing (`permissions: contents: read`). Findings are surfaced durably in **one tracking issue** (stable title `🏛️ Architecture review — codebase drift`): the run refreshes its body and appends a per-run comment, opening it on the first run. Has no issue/PR subject to move to `agent:blocked`, so a failed run comments the failure on the tracking issue (if open) and the Actions run itself fails. A manual dispatch can opt into Codex via the `provider` input; the cron path always uses the Claude default. Serialised by a single concurrency group (`cancel-in-progress: false`). |
