...
| `agent-implement-prd.yml` | `agent:implement` on a **PRD-shaped** issue | The PRD variant. Listens for the **same** `agent:implement` event and disambiguates purely by shape: it proceeds only when the issue **has** sub-issues (a leaf issue is silently skipped for `agent-implement.yml`), so exactly one of the two runs per issue — and both share the per-issue `agent-implement-issue-<N>` concurrency group so they never race. Runs `implement-prd` (lands the whole PRD as one branch, pulling the parent plus every sub-issue) + `write-prd-pr` (a body that `Closes` the PRD and each satisfied sub-issue), then pushes and opens the same auto-reviewed **draft** PR with identical `agent:in-progress` / `agent:blocked` / `agent:codex`-propagation / `agent:review`-via-`AGENT_PAT` behaviour as the normal path. An issue with an open PR already targeting it is refused. |
