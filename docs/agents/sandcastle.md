...
| `agent-to-issues.yml` | `agent:to-issues` on a PRD issue | Runs the `to-issues-prd` capability (the agent reads the PRD and emits a decomposition; it creates nothing), then the **workflow** does the `gh issue create` for each child and links it as a sub-issue of the PRD. Swaps `agent:to-issues` → `agent:in-progress` on start; comments the summary + created-issue list on the PRD on success; a failure lands the PRD in `agent:blocked` with a run-URL comment and always removes `agent:in-progress`. The parent PRD is never closed and its body is never edited. Refuses a sub-issue (a leaf, not a PRD) and a PRD that already has sub-issues (already decomposed). Applies **no** state label to the children — a human triages them. Per-PRD concurrency (`queue: max`, `cancel-in-progress: false`). |
